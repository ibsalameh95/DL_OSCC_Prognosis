...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, slide_id=None, slide_list_filename=None, cz_imgs_list= None, he_imgs_list= None, num_instances=16, imgs_type= 'HE'):
        self._slide_id = slide_id
        # self._transforms = torchvision.models.ResNet50_Weights.IMAGENET1K_V2.transforms() # 
        self._cz_transforms = torchvision.models.ResNet50_Weights.IMAGENET1K_V2.transforms() # self.cz_image_transforms()
        self._he_transforms = torchvision.models.ResNet50_Weights.IMAGENET1K_V2.transforms() # self.he_image_transforms()
    ...
    def construct_slide_bag(self, slide_id):
        ...
        return slide_images, slide_label, cz_img_paths, he_img_paths
        

    def __getitem__(self, idx):

        slide_images, slide_label, cz_img_paths, he_img_paths = self.construct_slide_bag(slide_id = self._slide_id)
        
        slide_label = torch.as_tensor(slide_label, dtype=torch.int64)

        return slide_images, slide_label, cz_img_paths, he_img_paths


def custom_collate_fn(batch):
    images_list, slide_label, cz_img_paths, he_img_paths = zip(*batch)

    return torch.cat(images_list,dim=0), torch.stack(slide_label,dim=0), cz_img_paths, he_img_paths
